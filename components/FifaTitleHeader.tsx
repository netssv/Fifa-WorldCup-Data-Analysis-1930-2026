...
/** Pulsing ring around the trophy that orbits on hover */
const OrbitDot: React.FC<{ delay: string }> = ({ delay }) => (
  <span
    className="absolute w-2 h-2 rounded-full bg-emerald-400/70 animate-orbit"
    style={{ animationDelay: delay }}
  />
);

/** Static trophy SVG — floats up/down continuously */
const FloatingTrophy: React.FC = () => (
  <div className="relative flex justify-center mb-6">
    {/* Orbit ring */}
    <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28 rounded-full border border-emerald-400/20" />
    <OrbitDot delay="0s" />
    <OrbitDot delay="-3s" />

    {/* Halo glow behind trophy */}
    <span className="absolute inset-0 flex items-center justify-center">
      <span className="w-16 h-16 rounded-full bg-emerald-500/10 blur-xl" />
    </span>

    {/* Trophy icon — floats */}
    <svg
      className="w-12 h-12 text-amber-400 relative z-10 drop-shadow-lg animate-float"
      viewBox="0 0 24 24"
      fill="currentColor"
    >
